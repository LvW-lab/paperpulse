...
def normalize_journal(name: str) -> str:
    # Keep normalization simple + consistent with DB/TQCC/whitelist
    return " ".join((name or "").strip().lower().split())


def load_whitelist(path: str) -> Set[str]:
    """
    whitelist.txt format: one journal per line (comments allowed with #)
    Values are normalized with normalize_journal().
    """
...
def load_tqcc_map(path: str) -> Dict[str, int]:
    """
    Loads a map: normalized journal name -> TQCC Value (int)
    CSV example: Rank;Journal;Value
    Note: CSV may be cp1252 if saved from Excel on Windows.
    """
    tqcc: Dict[str, int] = {}
    if not os.path.exists(path):
    ...
    # Try a couple encodings for robustness (Windows CSVs are often cp1252)
...
def filter_by_tqcc(
    ...
    for r in rows:
        ...
        # score can be 0 and that should be treated as a real score
        if score >= tqcc_min:
            kept.append(r)
        else:
            below += 1
...
def main() -> None:
    window_days = get_int_env("DAYS_OVERRIDE", 7)
    ...
    # Apply filtering if we have a tqcc map; whitelist still works alongside it
    if tqcc_map:
        rows = filter_by_tqcc(rows, tqcc_map, tqcc_min, include_unranked, whitelist)

    today_utc = datetime.now(timezone.utc).date().isoformat()
    subject = f"Weekly highlights — GBM invasion & integrins (as of {today_utc})"
